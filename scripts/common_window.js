function openPopWin(winURL__, winWidth__, winHeight__, openIfAlreadyOpened__, moveX__, moveY__, winFeatures__){
	...
	if (openIfAlreadyOpened__ == '') openIfAlreadyOpened__ = false;
	...
	try {
		while(i__ < popWins__.length) {
			if(popWins__[i__].closed) {
				...
				if (popWins__[i__].href__.length == winURL__.length && popWins__[i__].href__.toLowerCase().indexOf(winURL__.toLowerCase()) != -1) {
...
function openPopWinSimple(winURL, winWidth, winHeight, moveX, moveY, winFeatures) {
	if (openPopWinSimple.arguments.length == 6) winFeatures = "," + winFeatures;
	else winFeatures = "";
